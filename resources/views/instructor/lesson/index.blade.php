            ...
            <div class="col-xl-12 col-md-12 mb-4">
                <div class="card card-cascade narrower z-depth-1">
                    <div class="view gradient-card-header indigo narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{route('instructor.lesson.create', [$course->id, $section->id])}}" class="btn btn-outline-white btn-rounded btn-sm px-2"><i class="fa fa-pencil mt-0"></i></a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="example" class="table table-striped mb-0">
                                ...
                                <tbody>
                                    @foreach ($lessons as $lesson)
                                        <tr>
                                            <td><a href="{{ route('instructor.lesson.show', [$course->id, $section->id, $lesson->id]) }}" class="blue-text">{{$lesson->title}}</a></td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($lesson->status == 1)
                                                    ...
                                                    <form id="deactivate-form-{{$lesson->id}}" action="{{ route('instructor.lesson.status', [$course->id, $section->id, $lesson->id]) }}" method="post">
                                                ...
                                                @else
                                                    ...
                                                    <form id="activate-form-{{$lesson->id}}" action="{{ route('instructor.lesson.status', [$course->id, $section->id, $lesson->id]) }}" method="post">
                                                      @csrf {{method_field('PUT')}}
                                            ...
                                            <td>
                                                <a href="{{route('instructor.lesson.edit', [$course->id, $section->id, $lesson->id])}}" class="blue-text">Update</a> |
                                                ...
                                                <form id="delete-instructor-form-{{$lesson->id}}" action="{{ route('instructor.lesson.destroy', [$course->id, $section->id, $lesson->id]) }}" method="post">

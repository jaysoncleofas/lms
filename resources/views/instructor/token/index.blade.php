            ...
            <div class="col-xl-12 col-md-12 mb-4">
                <div class="card card-cascade narrower z-depth-1">
                    <div class="view gradient-card-header indigo narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center">
                            <a class="white-text mx-3">Token list</a>
                        <div>
                            <a href="{{route('instructor.token.create', $course->id)}}" class="btn btn-outline-white btn-rounded btn-sm px-2" data-toggle="tooltip" data-placement="top" title="Generate Token"><i class="fa fa-pencil mt-0"></i></a>
                            {{-- <a href="{{route('instructor.token.create', $course->id)}}" class="btn btn-outline-white btn-rounded btn-sm px-2" data-toggle="tooltip" data-placement="top" title="Generate Token" onclick="
                                        event.preventDefault();
                                        $('#generateToken{{$course->id}}').submit();
                                      "><i class="fa fa-pencil mt-0"></i></a>
                            <form id="generateToken{{$course->id}}" action="{{route('instructor.token.store', $course->id)}}" method="post">
                                {{ csrf_field() }}
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($token->status == 1)
                                                    ...
                                                    <form id="deactivate-form-{{$token->id}}" action="{{ route('instructor.token.update', [$course->id, $token->id]) }}" method="post">
                                                ...
                                                @else
                                                    ...
                                                    <form id="activate-form-{{$token->id}}" action="{{ route('instructor.token.update', [$course->id, $token->id]) }}" method="post">
                                                      @csrf {{method_field('PUT')}}
                                            ...
                                            <td>
                                                ...
                                                <form id="delete-token-form-{{$token->id}}" action="{{ route('instructor.token.destroy', [$course->id, $token->id]) }}" method="post">

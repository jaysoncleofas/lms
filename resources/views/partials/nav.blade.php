...
<nav class="navbar fixed-top navbar-toggleable-md navbar-expand-lg scrolling-navbar double-nav navbar-light bg-white">
    ...
    <ul class="nav navbar-nav nav-flex-icons ml-auto">
        ...
        <li class="nav-item">
            <a href="{{route('message.index')}}" class="nav-link" data-toggle="tooltip" data-placement="bottom" title="Messages"><i class="fa fa-envelope"></i> </a>
        </li>
        @endif
        <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <img src="{{ Auth::user()->avatar ? asset('storage/avatars/'.Auth::user()->avatar) : asset('images/profile_pic.png') }}" class="img-fluid rounded-circle z-depth-1" style="height:30px;width:30px;object-fit: cover;" alt=""> <span class="clearfix d-none d-sm-inline-block">{{Auth::user()->firstName.' '.Auth::user()->lastName}}</span>
            </a>
